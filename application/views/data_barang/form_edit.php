...
<div class="col-md-8">
    ...
    <div class="form-group row">
        <label for="kode_hs" class="col-sm-3 col-form-label">Kode HS</label>
        <div class="col-sm-6">
            <input type="text" class="form-control form-control-sm <?= (form_error('kode_hs')) ? 'is-invalid' : ''; ?>" id="kode_hs" name="kode_hs" placeholder="Kode HS" value="<?= (set_value('kode_hs')) ? set_value('kode_hs') : $barang->kode_hs; ?>">
            <div class="invalid-feedback">
        ...
        <div class="col-sm-6">
            <select class="custom-select custom-select-sm supplier <?= (form_error('kat_barang')) ? 'is-invalid' : ''; ?>" id="katbarangx" name="kat_barang">
                ...
                foreach ($kategori->result() as $k) :
                    $sup = (set_value('kat_barang')) ? set_value('kat_barang') : $barang->id_kat_barang;

                    $pilih = ($sup == $k->id_kat_barang) ? 'selected' : '';

                    echo '<option value="' . $k->id_kat_barang . '" ' . $pilih . '>
                        ' . $k->nama_kat_barang . '
                    </option>';
                endforeach; ?>
            </select>
	...
	<div class="form-group row">
        ...
        <div class="col-sm-6">
            <select class="custom-select custom-select-sm supplier <?= (form_error('kat_barang')) ? 'is-invalid' : ''; ?>" id="satuan" name="sat_barang">
                <option value="" disabled selected>Pilih Satuan</option>
                <?php
                foreach ($satuan as $key => $val) :
                    $sup = (set_value('sat_barang')) ? set_value('sat_barang') : $barang->satuan_barang;

                    $pilih = ($sup == $key) ? 'selected' : '';

                    echo '<option value="' . $key . '" ' . $pilih . '>
                        ' . $val . '
                    </option>';
                endforeach; ?>
            </select>
