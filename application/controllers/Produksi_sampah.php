...
class Produksi_sampah extends CI_Controller
{
    function __construct()
    {
        ...
        //load model
        $this->load->model('m_produksi_sampah');
    ...
    public function index()
    {
        ...
        $this->template->kasir('produksi_sampah/index', $data);
    }
    ...
    public function tambah_data()
    {
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if (!$this->cart->contents()) {
                ...
                redirect('tambah_produksi_sampah', 'refresh');
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $id = time();
                $noProduksi = $this->security->xss_clean($this->input->post('no-produksi-sampah', TRUE));
                ...
                $data_produksi_sampah = [
                    'id_produksi_sampah' => $id,
                    'no_produksi_sampah' => $noProduksi,
                    'tgl_produksi_sampah' => $tgl,
                    'id_user' => $user,
                    'tipe_produksi' => '3',
                ...
                foreach ($this->cart->contents() as $c) {
                    $item = [
                        'id_produksi_sampah' => $data_produksi_sampah['id_produksi_sampah'],
                        'id_barang' => $c['id'],
                        'qty' => $c['qty']
                    ];
                ...
                $simpan = $this->m_produksi_sampah->save('tbl_produksi_sampah', $data_produksi_sampah);

                if ($simpan) {
                    //simpan data detail produksi
                    $this->m_produksi_sampah->multiSave('tbl_detail_produksi_sampah', $cart);
                    //kosongkan cart
                    $this->cart->destroy();
                    //buat notifikasi penyimpanan berhasil
                    $this->session->set_flashdata('success', 'Data Sampah Produksi berhasil ditambahkan...');

                    redirect('produksi_sampah');
                }
            }
        }

        $data = [
            'title' => 'Tambah Data Sampah Produksi',
            'data' => $this->m_produksi_sampah->getData('tbl_barang', ['active' => 'Y']),
            'produksi_proses' => $this->m_produksi_sampah->getAllData('tbl_produksi'),
            'table' => $this->read_cart()
        ];

        $this->template->kasir('produksi_sampah/form_input', $data);
    }
    ...
    public function hapus_data()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $hapus = $this->m_produksi_sampah->delete(['tbl_produksi_sampah', 'tbl_detail_produksi_sampah'], ['id_produksi_sampah' => $id]);
    ...
    public function detail_produksi_sampah($id = null)
    {
        if ($id == null) {
            redirect('data_produksi_sampah');
        }

        //cek login
        $this->is_login();
        //ambil data
        $getData = $this->m_produksi_sampah->getDataProduksiSampah($this->security->xss_clean($id));
        ...
        $data = [
            'title' => 'Detail Sampah Produksi ' . $id,
            'data' => $getData
        ];

        $this->template->kasir('produksi_sampah/detail', $data);
    }

    public function edit_produksi_sampah($id = null)
    {
        if ($id == null) {
            redirect('produksi_sampah');
        }
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if (!$this->cart->contents()) {
                ...
                redirect('edit_produksi_sampah/' . $id, 'refresh');
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $update = $this->m_produksi_sampah->update('tbl_produksi_sampah', $data_produksi_sampah, ['id_produksi_sampah' => $idP]);

                if ($update) {
                    //hapus detail produksi
                    $this->m_produksi_sampah->delete('tbl_detail_produksi_sampah', ['id_produksi_sampah' => $idP]);
                    //simpan data detail produksi
                    $this->m_produksi_sampah->multiSave('tbl_detail_produksi_sampah', $cart);
                    //kosongkan cart
                    $this->cart->destroy();
                    //buat notifikasi penyimpanan berhasil
                    $this->session->set_flashdata('success', 'Data Sampah roduksi berhasil diperbarui...');
        ...
        $data = [
            'title' => 'Edit Data Sampah Produksi',
            'fdata' => $fData,
            'data' => $this->m_produksi_sampah->getData('tbl_barang', ['active' => 'Y']),
            'table' => $this->read_cart()
        ];

        $this->template->kasir('produksi_sampah/form_edit', $data);
    }

    public function tambah_cart_produksi_sampah()
    {
        //cek login
        $this->is_login();
        //validasi request ajax
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $get_barang = $this->m_produksi_sampah->getData('tbl_barang', $where);
    ...
    public function get_item_produksi_sampah($uri = null)
    {
        $this->is_login();
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($get_item) {
                ...
                $getBarang = $this->m_produksi_sampah->getData('tbl_barang', ['kode_barang' => $get_item['id']]);
                ...
                if ($getBarang->num_rows() != 1) {
                    ...
                    if (strtolower($uri) == 'edit_produksi_sampah') {
    ...
    public function update_cart_produksi_sampah()
    {
        //cek login
        $this->is_login();
        //validasi request ajax
    ...
    public function hapus_cart_produksi_sampah()
    {
        //cek login
        $this->is_login();
        //validasi request ajax
    ...
    public function ajax_produksi_sampah()
    {
        $this->is_login();
        //cek apakah request berupa ajax atau bukan, jika bukan maka redirect ke home
        if ($this->input->is_ajax_request()) {
            //ambil list data
            $list = $this->m_produksi_sampah->get_datatables();
            ...
            foreach ($list as $i) {
                ...
                if ($this->session->userdata('level') == 'admin' || $this->session->userdata('UserID') == $i->id_user) :
                    ...
                    $button .= '<a href="' . site_url('edit_produksi_sampah/' . $i->id_produksi_sampah) . '" class="btn btn-warning btn-sm text-white">Edit</a>
                        <button type="button" class="btn btn-danger btn-sm"onclick="hapus_produksi_sampah(\'' . $i->id_produksi_sampah . '\')">Hapus</button>';
                ...
                $row = array();
                $row[] = $no;
                $row[] = $i->no_produksi_sampah;
                $row[] = $this->tanggal_indo($i->tgl_produksi_sampah);
                $row[] = $i->jumlah;
                $row[] = $i->fullname;
                // $row[] = '<span class="pr-3">' . number_format($i->total, 0, ',', '.') . ',-</span>';
                $row[] = '<a href="' . site_url('data_produksi_sampah/' . $i->id_produksi_sampah) . '" class="btn btn-sm btn-success">Detail</a>
                ' . $button;
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->m_produksi_sampah->count_all(),
                "recordsFiltered" => $this->m_produksi_sampah->count_filtered(),
                "data" => $data
            );
        ...
        if ($this->cart->contents()) {
            ...
            foreach ($this->cart->contents() as $c) {
                ...
                $table .= '<td class="text-center">
                                <button type="button" class="btn btn-warning btn-sm text-white" onclick="get_item_produksi_sampah(\'' . $c['rowid'] . '\')">Edit</button>
                                <button type="button" class="btn btn-danger btn-sm text-white" onclick="remove_item_produksi_sampah(\'' . $c['rowid'] . '\')">Hapus</button>
                            </td></tr>';
            }
        } else {
            $table = '<tr>
