...
class Data_barang extends CI_Controller
{
    ...
    public function tambah_data()
    {
			...
			*/
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $simpan = array(
					...
					'id_kat_barang' => $this->security->xss_clean($this->input->post('kat_barang', TRUE)),
					'satuan_barang' => $this->security->xss_clean($this->input->post('sat_barang', TRUE)),
                    'kode_hs' => $this->security->xss_clean($this->input->post('kode_hs', TRUE))
                );
        ...
        $data = [
			...
			'kategori' => $this->m_barang->getAllData('tbl_kat_barang'),
			'satuan' => ['KGM' => 'KGM', 'YRD' => 'YRD', 'PCE' => 'PCE', 'MTR' => 'MTR']
        ];
    ...
    public function ajax_barang()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($list as $i) {
                ...
                $row[] = $no;
                $row[] = $i->kode_barang;
                $row[] = substr($i->kode_hs,0,4).'.'.substr($i->kode_hs,4,2).'.'.substr($i->kode_hs,6);
                $row[] = $i->nama_kat_barang;
                $row[] = $i->nama_barang;
                $row[] = $i->brand;
                $row[] = $i->stok.' <span class="float-right">'.$i->satuan_barang.'</span>';
