...
class Produksi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->kasir('produksi/index', $data);
    }
    ...
    public function tambah_data()
    {
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if (!$this->cart->contents()) {
                ...
                redirect('tambah_produksi_proses', 'refresh');
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                foreach ($this->cart->contents() as $c) {
                    $item = [
                        'id_produksi' => $data_produksi_proses['id_produksi'],
                        'id_barang' => $c['id'],
                        'qty' => $c['qty']
                    ];
                ...
                $simpan = $this->m_produksi->save('tbl_produksi', $data_produksi_proses);

                if ($simpan) {
                    //simpan data detail produksi
                    $this->m_produksi->multiSave('tbl_detail_produksi', $cart);
                    //kosongkan cart
                    $this->cart->destroy();
                    //buat notifikasi penyimpanan berhasil
                    $this->session->set_flashdata('success', 'Data Produksi berhasil ditambahkan...');

                    redirect('produksi_proses');
                }
            }
        }

        $data = [
            'title' => 'Tambah Data Prduksi Proses',
            'data' => $this->m_produksi->getData('tbl_barang', ['active' => 'Y']),
            'table' => $this->read_cart()
        ];

        $this->template->kasir('produksi/form_input', $data);
    }
    ...
    public function hapus_data()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $hapus = $this->m_produksi->delete(['tbl_produksi', 'tbl_detail_produksi'], ['id_produksi' => $id]);
    ...
    public function detail_produksi_proses($id = null)
    {
        if ($id == null) {
            redirect('data_produksi_proses');
        }

        //cek login
        $this->is_login();
        //ambil data
        $getData = $this->m_produksi->getDataProduksiProses($this->security->xss_clean($id));
        ...
        $data = [
            'title' => 'Detail Produksi Proses ' . $id,
            'data' => $getData
        ];

        $this->template->kasir('produksi/detail', $data);
    }

    public function edit_produksi_proses($id = null)
    {
        if ($id == null) {
            redirect('produksi_proses');
        }
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if (!$this->cart->contents()) {
                ...
                redirect('edit_produksi_proses/' . $id, 'refresh');
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $update = $this->m_produksi->update('tbl_produksi', $data_produksi_proses, ['id_produksi' => $idP]);

                if ($update) {
                    //hapus detail produksi
                    $this->m_produksi->delete('tbl_detail_produksi', ['id_produksi' => $idP]);
                    //simpan data detail produksi
                    $this->m_produksi->multiSave('tbl_detail_produksi', $cart);
                    //kosongkan cart
                    $this->cart->destroy();
        ...
        $data = [
            'title' => 'Edit Data Produksi Proses',
            'fdata' => $fData,
            'data' => $this->m_produksi->getData('tbl_barang', ['active' => 'Y']),
            'table' => $this->read_cart()
        ];

        $this->template->kasir('produksi/form_edit', $data);
    }

    public function tambah_cart_produksi_proses()
    {
        //cek login
        $this->is_login();
        //validasi request ajax
    ...
    public function get_item_produksi_proses($uri = null)
    {
        $this->is_login();
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($get_item) {
                ...
                $getBarang = $this->m_produksi->getData('tbl_barang', ['kode_barang' => $get_item['id']]);
                ...
                if ($getBarang->num_rows() != 1) {
                    ...
                    if (strtolower($uri) == 'edit_produksi_proses') {
    ...
    public function update_cart_produksi_proses()
    {
        //cek login
        $this->is_login();
        //validasi request ajax
    ...
    public function hapus_cart_produksi_proses()
    {
        //cek login
        $this->is_login();
        //validasi request ajax
    ...
    public function ajax_produksi_proses()
    {
        $this->is_login();
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($list as $i) {
                ...
                if ($this->session->userdata('level') == 'admin' || $this->session->userdata('UserID') == $i->id_user) :
                    ...
                    $button .= '<a href="' . site_url('edit_produksi_proses/' . $i->id_produksi) . '" class="btn btn-warning btn-sm text-white">Edit</a>
                        <button type="button" class="btn btn-danger btn-sm"onclick="hapus_produksi_proses(\'' . $i->id_produksi . '\')">Hapus</button>';
                ...
                $row[] = $no;
                $row[] = $i->no_produksi;
                $row[] = $this->tanggal_indo($i->tgl_produksi);
                $row[] = $i->jumlah;
                $row[] = $i->fullname;
                // $row[] = '<span class="pr-3">' . number_format($i->total, 0, ',', '.') . ',-</span>';
                $row[] = '<a href="' . site_url('data_produksi_proses/' . $i->id_produksi) . '" class="btn btn-sm btn-success">Detail</a>
                ' . $button;
        ...
        if ($this->cart->contents()) {
            ...
            foreach ($this->cart->contents() as $c) {
                ...
                $table .= '<td class="text-center">
                                <button type="button" class="btn btn-warning btn-sm text-white" onclick="get_item_produksi_proses(\'' . $c['rowid'] . '\')">Edit</button>
                                <button type="button" class="btn btn-danger btn-sm text-white" onclick="remove_item_produksi_proses(\'' . $c['rowid'] . '\')">Hapus</button>
                            </td></tr>';
            }
        } else {
            $table = '<tr>
