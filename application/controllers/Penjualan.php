...
class Penjualan extends CI_Controller
{
    ...
    public function tambah_data()
    {
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($simpan) {
                    ...
                    $this->session->set_flashdata('success', 'Data pengiriman barang berhasil ditambahkan...');
        ...
        $data = [
            'title' => 'Tambah Data Pengiriman Barang',
            'data' => $this->m_penjualan->getData('tbl_barang', ['active' => 'Y']),
            'pabean' => $this->m_penjualan->getData('tbl_pabean', ['jenis_dok' => '0']),
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($update) {
                    ...
                    $this->cart->destroy();
                    //buat notifikasi penyimpanan berhasil
                    $this->session->set_flashdata('success', 'Data pengiriman barang berhasil diperbarui...');
    ...
    public function peb()
    {
        ...
        $data = [
            'title' => 'Data Pabean PEB'
        ];

        $this->template->kasir('penjualan/peb', $data);
    }

    public function ajax_peb()
    {
        $this->is_login();
        //cek apakah request berupa ajax atau bukan, jika bukan maka redirect ke home
        if ($this->input->is_ajax_request()) {
            //ambil list data
            $list = $this->m_penjualan->get_datatables_peb();
            //siapkan variabel array
            $data = array();
            $no = $_POST['start'];
            ...
            foreach ($list as $i) {
                ...
                if ($this->session->userdata('level') == 'admin' || $this->session->userdata('UserID') == $i->id_user) :
                    ...
                    $button .= '<a href="' . site_url('edit_peb/' . $i->id_pabean) . '" class="btn btn-warning btn-sm text-white">Edit</a>
                        <button type="button" class="btn btn-danger btn-sm"onclick="hapus_peb(\'' . $i->id_pabean . '\')">Hapus</button>';
                ...
                // $row[] = '<span class="pr-3">' . number_format($i->total, 0, ',', '.') . ',-</span>';
                $row[] = '<a href="' . site_url('data_peb/' . $i->id_pabean) . '" class="btn btn-sm btn-success">Detail</a>
                ' . $button;
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->m_penjualan->count_all_peb(),
                "recordsFiltered" => $this->m_penjualan->count_filtered_peb(),
                "data" => $data
            );
            //output to json format
            echo json_encode($output);
    ...
    public function detail_peb($id = null)
    {
        if ($id == null) {
            redirect('data_peb');
        }
        ...
        $this->is_login();
        //ambil data
        $getData = $this->m_penjualan->getDataPeb($this->security->xss_clean($id));
        ...
        $this->template->kasir('penjualan/detailpeb', $data);
    }
    ...
    public function tambah_peb()
    {
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            */
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                //simpan ke database
                $save = $this->m_penjualan->save('tbl_pabean', $simpan);

                if ($save) {
                    $this->session->set_flashdata('success', 'Data Pabean PEB berhasil ditambah...');
        ...
        $data = [
            ...
            'data' => $this->m_penjualan->getData('tbl_barang', ['active' => 'Y']),
            'kode_dok' => array('BC30', 'BC41')
        ];

        $this->template->kasir('penjualan/form_input_peb', $data);
    }
    ...
    public function update_peb($id = '')
    {
        ...
        //ambil data barang
        $pabean = $this->m_penjualan->getDataPeb($id);
        ...
        if ($this->input->post('update', TRUE) == 'Update') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                //simpan ke database
                $up = $this->m_penjualan->update('tbl_pabean', $update, ['id_pabean' => $this->security->xss_clean($this->input->post('id', TRUE))]);

                if ($up) {
                    $this->session->set_flashdata('success', 'Data Pabean PEB berhasil diperbarui...');
        ...
        $data = [
            'title' => 'Edit Data Pabean PEB',
            'pabean' => $pabean->row(),
            'kode_dok' => array('BC30', 'BC41')
        ];

        $this->template->kasir('penjualan/form_edit_peb', $data);
    }

    public function hapus_data_peb()
    {
        //cek login
        $this->is_login();
        //validasi request ajax
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $hapus = $this->m_penjualan->delete('tbl_pabean', ['id_pabean' => $id]);
