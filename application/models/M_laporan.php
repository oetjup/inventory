...
class M_laporan extends CI_Model
{
    ...
    function getDataStokHarian($tanggal, $tanggal2, $where = null, $page, $offset)
    {
        // $table = 'tbl_barang b
        //             LEFT JOIN
        ...
        //             LEFT JOIN
        //             (SELECT qty, id_barang FROM tbl_penjualan pn
        //             LEFT JOIN tbl_detail_penjualan dpn ON(pn.id_penjualan = dpn.id_penjualan AND tgl_penjualan < \'' . $tanggal . '\')) AS x ON(b.kode_barang = x.id_barang)
        //             LEFT JOIN
        //             (SELECT qty, id_barang FROM tbl_pembelian pm
        //             LEFT JOIN tbl_detail_pembelian dpm ON(pm.id_pembelian = dpm.id_pembelian AND tgl_pembelian < \'' . $tanggal . '\')) AS y ON(b.kode_barang = y.id_barang)
        //             LEFT JOIN tbl_kat_barang kb ON(b.id_kat_barang = kb.id_kat_barang)';

        $table = 'tbl_barang b
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_penjualan pn
                        JOIN tbl_detail_penjualan dpn ON(pn.id_penjualan = dpn.id_penjualan AND (tgl_penjualan BETWEEN \'' . $tanggal . '\' AND \'' . $tanggal2 . '\')) GROUP BY id_barang) AS c ON(b.kode_barang = c.id_barang)
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_pembelian pm
                        JOIN tbl_detail_pembelian dpm ON(pm.id_pembelian = dpm.id_pembelian AND (tgl_pembelian BETWEEN \'' . $tanggal . '\' AND \'' . $tanggal2 . '\')) GROUP BY id_barang) AS d ON(b.kode_barang = d.id_barang)
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_penjualan pn
                        JOIN tbl_detail_penjualan dpn ON(pn.id_penjualan = dpn.id_penjualan AND tgl_penjualan > \'' . $tanggal2 . '\') GROUP BY id_barang) AS e ON(b.kode_barang = e.id_barang)
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_pembelian pm
                        JOIN tbl_detail_pembelian dpm ON(pm.id_pembelian = dpm.id_pembelian AND tgl_pembelian > \'' . $tanggal2 . '\') GROUP BY id_barang) AS f ON(b.kode_barang = f.id_barang)
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_penjualan pn
                        JOIN tbl_detail_penjualan dpn ON(pn.id_penjualan = dpn.id_penjualan AND tgl_penjualan < \'' . $tanggal . '\') GROUP BY id_barang) AS x ON(b.kode_barang = x.id_barang)
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_pembelian pm
                        JOIN tbl_detail_pembelian dpm ON(pm.id_pembelian = dpm.id_pembelian AND tgl_pembelian < \'' . $tanggal . '\') GROUP BY id_barang) AS y ON(b.kode_barang = y.id_barang)
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_produksi pr
                        JOIN tbl_detail_produksi dpr ON(pr.id_produksi = dpr.id_produksi AND (tgl_produksi BETWEEN \'' . $tanggal . '\' AND \'' . $tanggal2 . '\')) GROUP BY id_barang) AS g ON(b.kode_barang = g.id_barang)
                        LEFT JOIN
                        (SELECT SUM(qty) AS qty, id_barang FROM tbl_produksi pr
                        JOIN tbl_detail_produksi dpr ON(pr.id_produksi = dpr.id_produksi AND tgl_produksi > \'' . $tanggal2 . '\') GROUP BY id_barang) AS h ON(b.kode_barang = h.id_barang)
                        LEFT JOIN tbl_kat_barang kb ON(b.id_kat_barang = kb.id_kat_barang)
                        ';

        // $select = 'kode_barang, nama_barang, brand, stok, SUM(c.qty) AS qty_penjualan, SUM(d.qty) AS qty_pembelian, SUM(e.qty) AS qty_penjualan_new, SUM(f.qty) AS qty_pembelian_new, nama_kat_barang, SUM(x.qty) AS qty_penjualan_bef, SUM(y.qty) AS qty_pembelian_bef';

        $select = 'kode_barang, nama_barang, brand, stok, c.qty AS qty_penjualan, d.qty AS qty_pembelian, e.qty AS qty_penjualan_new, f.qty AS qty_pembelian_new, nama_kat_barang, x.qty AS qty_penjualan_bef, y.qty AS qty_pembelian_bef, g.qty AS qty_proses, h.qty AS qty_proses_new';
    ...
    {
        $select = 'p.id_pembelian AS id_pembelian, nama_barang, brand, dp.harga AS harga, qty, satuan_barang, hpp, bm, bmt, pph, ppn, nama_supplier, (SELECT COUNT(*) FROM tbl_detail_pembelian WHERE id_pembelian = p.id_pembelian) AS row';
